...
// SiteID returns the site identifier for API calls.
// Controller v6 uses "id", v5 uses "key" or "name".
func (s *Site) SiteID() string {
	if s.Id != "" {
		return s.Id
	}
	if s.Key != "" {
...
func (c *Client) Sites() ([]*Site, error) {
	...
	if currentPage > maxPages {
		slog.Warn("stopped fetching sites after too many pages",
			"pages", maxPages,
			"sites", len(sites),
		)
	}
